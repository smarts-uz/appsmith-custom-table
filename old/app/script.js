...
function App() {
    ...
    const hasNextPage = infiniteQueryData ? infiniteQueryData[infiniteQueryData.length - 1]?.length === pagination.pageSize : false;
    ...
    const columns = React.useMemo(() => {
        ...
        const pinColumn = [];
        if (enableRowPinning) {
            pinColumn.push(columnHelper.display({
                id: 'pin',
                header: t('pinRow'),
                size: 60,
                cell: ({ row }) => (
        ...
        const autoCols = Object.keys(tableData[0]).map((colKey) => {
            if (!schema[colKey]) {
            ...
            if (type === 'number' || type === 'currency') {
                const columnValues = tableData.map(row => row[colKey]).filter(v => typeof v === 'number' && isFinite(v));
        ...
        if (rowIndexColumn?.enable && rowActions.length > 0) {
            actionColumns.push(columnHelper.display({
                ...
                cell: ({ row, table, column }) => {
                    ...
                    return (
                        <div className={`dropdown ${isDropdownOpen ? 'dropdown-open' : ''} ${dropdownPositionClass} w-full h-full`}>
                            <div tabIndex={0} role="button" className="btn w-full h-full btn-ghost hover:bg-transparent" onClick={toggleDropdown}>
        ...
        console.log('Cols', [...pinColumn, ...indexColumns, ...autoCols, ...actionColumns])

        return [...pinColumn, ...indexColumns, ...autoCols, ...actionColumns]
    }, [tableData, schema, rowActions, actionSize, openDropdownRowId, rowIndexPin, rowIndexSize, enablePagination, t, enableRowPinning])
    ...
    const RowRenderer = React.useCallback(({ row, index }) => {
        ...
        const trProps = {
            key: row.id,
            className: `border-b border-base-200 ${row.getIsSelected() ? 'bg-primary/10' : ''}`,
            style: { ...conditionalStyle },
        };

        if (isPinned) {
            // A row's height is assumed to be 41px and header is 49px. This might need adjustment if row styles change.
            const top = isPinned === 'top' ? `${49 + row.getPinnedIndex() * 41}px` : undefined;
            const bottom = isPinned === 'bottom' ? `${(table.getBottomRows().length - 1 - row.getPinnedIndex()) * 41}px` : undefined;
            ...
            trProps.className += ` ${index % 2 === 0 ? 'even-row' : 'odd-row'} cursor-pointer`;
            trProps.onClick = () => row.toggleSelected();
        }

        const cells = row.getVisibleCells().map((cell, cellIndex) => {
            const sizeClass = `col-size-${cell.column.columnDef.meta?.size || 'md'}`;
            const baseStyle = getCommonPinningStyles(cell.column);
            const isLastCell = cellIndex === row.getVisibleCells().length - 1;

            const { textAlign = 'left', cellPadding = 8 } = cell.column.columnDef.meta || {};
    ...
    if (isError) {
        return <div className="text-center p-8 text-lg text-red-600">{t('error')} {(error.code)} {(error.message)} </div>;
    }

    if (isLoading) {
        return (
            <div className="text-center flex justify-center h-full p-8 items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-loader-circle-icon lucide-loader-circle animate-spin">
    ...
    return (
        <div data-theme="white" className="font-sans flex flex-col h-full" style={themeStyles}>
            ...
            <div className="flex-grow p-2 overflow-hidden">
                <div className="h-full overflow-auto shadow-sm rounded-lg" style={showBorder ? { border: `1px solid var(--border-color)` } : {}}>
                    <table className="table table-fixed table-pin-rows table-pin-cols w-full" style={{ minWidth: table.getTotalSize() }}>
                        <thead className="bg-base-200 text-base-content sticky" style={{ top: 0, zIndex: 20 }}>
            ...
            <div className="p-2">
                {enablePagination ? (
                    ...
                    <div ref={loadMoreRef} className="text-center p-4 flex flex-col items-center gap-2">
                        {isFetchingNextPage ? (
